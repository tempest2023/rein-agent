...
> 版本：v0.1 · 产品需求草案\
> 日期：2026-09-22\
> 文档状态：已确认 OpenClaw、P0 平台为 Slack、成员／董事／可用资金以组织自有数据库为准、Board 仅投赞成票（approve-only），以及按提案类型配置候选名额与每人批准额度。严格的 P0 MVP 范围见 §2.3；本文其他内容（含各项数值默认值）仍为产品建议，尚未批准或上线。\
...
本产品是常驻 Slack（已确认的 P0 平台）的社群运营助手。Slack 面向核心 Board Member 与 Core Contributor，承担投票表决、资金审核和活动审核等内部治理工作。后续的 Discord 入口面向一般参与者，早期仅提供入门引导、免费资源导航和参与组织建设的路径（C16）。成员通过自然语言与 Agent 交流。官网承担活动发现、报名入口、成果展示和组织透明度建设；管理后台承担规则配置、例外处理和运营监督。
...
若同一活动申请经费，则进入周期性评选。Agent 向 Board 介绍提案和资金情况，主持限时的简单 Board 投票，并记录结果。
...
- 一期活动经费评选以组织自有数据库中的成员、董事与可用资金为准，由每名有资格董事投一票，作为未来治理演进的过渡。不得把一期结果描述成已上线的 DAO 或链上治理。
...
| C02 | 通过 Slack（已确认的 P0 平台）接入，成员可在群里 @Agent 发起与推进事项；第二平台属于后续阶段。 |
...
| C06 | 申请组织经费的活动进入评选队列，由 Board 进行线上批准投票。 |
| C07 | 每周或每两周组织评选，投票窗口支持较短时长，由 Agent 主持并通知 Board；具体节奏与窗口时长仍待确认。 |
| C08 | 仅有资格的 Board Member 可以投票，每名有资格成员权重相同；P0 只投赞成票（approve-only）：没有反对选项，弃权等于不投任何赞成票，每个提案类型各自设定每人批准额度（通常为 1，某些类型允许多个），得票（赞成票）最高者通过。全员弃权的一轮不产生赢家；加权投票属于后续阶段。 |
...
| C14 | Agent 每轮从「近期提案 + 此前未入选提案」组成的候选池中组装投票名单。每个提案类型各有自己的候选名额上限，由运营方按有资格投票人数量确定；Events 当前示例为 10，但这不是全局固定政策。只有一个候选的一轮同样有效。 |
| C15 | 结果公布后，反馈可以修改已通过的提案。涉及预算、地点、人员或活动主流程的变更，需要至少一名现任 Board Member 批准后，改动版本才能生效；当前实现把日程（schedule）也视为重大字段。只改标题或摘要的修订属于普通修订，Agent 可以接受其中合理的建议并自行使其生效，无需另行取得 Board 批准。 |
| C16 | Slack 只面向核心 Board Member 和 Core Contributor，承载投票、资金审核和活动审核等内部治理。Discord 面向一般参与者，早期 Agent 只做入门引导、免费资源导航和参与路径，不执行治理操作。Slack 仍是唯一的 P0 MVP 平台；Discord 属于后续独立范围，当前既未接入也未实现，P0 不依赖它。身份关联按平台和所属空间分别管理；Discord 身份不得自动取得 Slack 治理权限，一个平台的角色或频道成员资格也不能代替另一平台的资格校验。现阶段不建设通用的跨 Channel 业务框架。 |
...
上述建议大部分不属于 MVP；§2.3 列出被延后的范围及原因。

### 2.3 P0 MVP 的严格范围、延后项与异议

组织希望先在真实数据上做出一个可运行的纵向切片，再考虑本文的其余内容。已确认的 MVP 为：

1. **Slack 身份关联。** 管理员把可信的 Slack 团队／用户对关联到组织数据库中的一条社群记录。未关联者可提问、可起草，但不能提交提案、投票或执行操作。一个部署只服务一个 Slack 工作区，因为运行时的可信发送者上下文不携带团队 ID。
2. **Contributor 提案。** 已关联且有效的 Contributor 在 Slack 提交一条简单的经费提案，Agent 保存其本人确认的版本。
3. **Board 批准投票与结果。** 有资格的董事在 Slack 收到候选池并投出明确赞成票或弃权，没有反对选项。每名有资格成员权重相同，适用按类型配置的每人批准额度，赞成票最高者通过；全员弃权的一轮不产生赢家。Agent 记录参与情况与所适用的规则，并把结果返回当前调用回合。**把结果自动回帖到 Slack 尚未实现**：当前结果工具只返回结果，不发消息。
4. **只读资金快照。** Agent 可以展示最新一条人工录入的可用资金快照，含币种与记录时间；它不能创建、占用或支出资金。

第 1、4 步的本地只读组件已有测试覆盖，但尚未连接任何真实数据库。第 2、3 步的写入工具已存在，且只在显式 `mvp` 配置块下注册。开票工具从已存的投票类型取候选名额，并自行从库中提案组装候选池（含此前未入选的提案），因此调用方无法给出候选、名额或选项名；投票工具只接受赞成票，上限为该轮投票自身的批准额度，空数组即弃权；候选名单与两个上限都在写库时由数据库冻结。**C08、C14 因此已有注册的代码路径**；剩余缺口是尚未批准的具体名额与批准额度数值，以及真实数据库与 Slack 工作区均未接入。

**C15 现在已有面向 Agent 的注册路径，但目前只由本地测试演练。** 写入层 `foundation-db-writer.ts` 提供修订记录与 `rein_mvp_approve_revision` 批准调用，数据库触发器对命中预算、地点、人员或主流程等重大字段的修订拒绝生效，直到记录到现任董事的批准。`mvp-feedback-tools.ts` 在同一个显式 `mvp` 配置块下注册三个工具，因此该流程不再只有数据库层闸门：`rein_mvp_proposal_comment_suggest` 记录评论或修订建议，`rein_mvp_revision_approve` 记录现任董事的批准，`rein_mvp_revision_apply` 使已记录的修订成为生效版本。**生效这一步正是规则两侧的分界**：只改标题或摘要的修订属于**普通修订**，Agent 可以接受其中合理的建议并自行使其生效，无需另行取得 Board 批准；涉及预算、地点、日程、人员或活动主流程的修订属于**重大修订**，在记录到现任董事的批准之前会被 `revision_not_approved` 拒绝，当前实现把日程视为重大字段。评论与修订建议都要求在已批准的 Board 频道内、且发送者本人是现任董事；数据库行另外允许**有效 Contributor** 作为修订作者，这是数据库层的放宽，当前注册的工具没有把它开放出来。以上全部只由本地合成测试演练：没有任何迁移被应用、没有连接真实数据库、也没有任何 Slack 工作区跑过该流程，因此这里没有端到端结果。

通过经费提案只是决策记录：不会转账，不构成付款承诺，也不单独构成支出授权。

MVP 明确不做：

| 延后项 | 现在延后的原因 |
| --- | --- |
| 加权投票、参与门槛与法定人数 | MVP 计票只需“每名有资格成员一票”，不需要这些算术与政策。 |
| 回避与利益冲突规则 | 需要尚未形成的披露与资格政策。 |
| 多提案之间的资金分配与排序 | 需要事先批准的排序规则；绝不能按消息处理先后决定。 |
| 付款、报销与结算 | Agent 不执行付款；MVP 中财务只读。 |
| 活动空间、清单、提醒、成果与文章 | 属于另一块产品面，且依赖 MVP 不必等待的政策。 |
| 官网发布与社交媒体 | 需要提供方契约，以及素材授权与保留期限的决定。 |
| 周报、异常中心、暂停控制与监督 | 应跟随上述流程，而不是先行。 |
| 第二聊天平台、跨平台身份、DAO／链上迁移 | P0 是单一平台加组织自有数据库。 |

对当前草案的异议：第 5–13 节描述的范围接近一个完整产品，其中大量内容依赖组织尚未批准的治理参数（权重、法定人数、回避、预算竞争）。现在实现这些路径只会固化未批准的政策，并不会让第一个切片更早可用。建议先在真实数据上完成上述四个 MVP 步骤，之后每放开一项延后内容，都先把它对应的规则与责任人记入[决策登记册](decisions.md)。

有两条边界必须如实说明。最高票并列的平票规则尚未采用：并列时 Agent 不得宣布正式赢家，也不得私自打破平局，具体口径仍待确认。全员弃权的一轮与平票不同，属已定规则：不产生赢家。另外，通过提案不等于资金移动：投票只产生决策记录，付款与记账由人在 Agent 之外完成。
...
| 阶段 | 范围 | 交付结果 |
| --- | --- | --- |
| P0 MVP（§2.3）：最小运营切片 | Slack 身份关联；简单的 Contributor 经费提案；简单的 Board 投票与结果发布；只读的可用资金快照 | 已关联的 Contributor 能提交提案，Board 能做出决定，决策被记录并发布，无需发起人推动 |
| P0 其余部分：待 MVP 切片验证后再开放 | 零预算快速通道；活动空间；筹备提醒；成果验收；官网文章；异常监督与周报 | 一场活动从提案到公开成果能完整完成，常规过程不依赖发起人逐步推动 |
...
已选定 Slack 作为 P0 平台，服务核心治理成员。Discord 是后续面向一般参与者的独立入口，初期仅承担入门与资源导航；两个平台不会因本 PRD 而同时上线。
...
- 结果公布后，反馈仍可修改已通过的提案。只改标题或摘要的修订属于普通修订，Agent 可接受其中合理的建议并自行使其生效，无需另行取得 Board 批准；涉及预算、地点、人员或活动主流程的修订须先取得至少一名现任 Board Member 的批准，当前实现把日程也视为重大字段。两种情形都以修订记录留存作者与时间，且修订只是请求，永不构成支出授权。
...
若提案填写“申请金额为零”，却仍期望报销或组织付款，应视为金额未补齐的资金申请，不得走零预算快速通道；负责人明确申请金额后方可进入评选轮次。

**R09 · 评估异议与复核（P0）**

负责人可以询问理由、纠正事实或请求复核。Agent 记录原结论、补充材料及处理结果。人工修正后的同一事实，不得在没有新依据时被 Agent 反复改回。

### 8.4 周期评选与 Board 批准投票

**MVP 状态。** §2.3 的 MVP 使用已确认的「只投赞成票」计票，只需 R10（可见的截止时间、组装后的候选名单与资格）、R11（基于事实的简报）、R12（明确且可归属的赞成票）和 R16（截止、结果与异常）。已确认形态见 C08、C14、C15：只投赞成票、按提案类型以有资格投票人数为基准设置候选名额、按类型设置每人批准额度、只改标题或摘要的普通修订由 Agent 自行接受并生效，以及预算、地点、人员或活动主流程（当前实现含日程）变更须先取得现任 Board Member 批准。R13、R14、R15 涉及加权投票、法定人数、回避与预算竞争，仍属建议默认值，已从 MVP 延后；不得把它们描述成已采用。
...
- **组装本轮名单。** 每轮从「近期提案 + 此前未入选提案」组成候选名单。按提案类型适用各自的候选名额上限；Events 示例为 10，由运营方结合有资格投票人数量确定，不是全局固定数字。只有一个候选的一轮同样有效。
...
- 仅支持**赞成**与**弃权**，没有反对选项；弃权不产生任何赞成票。每名投票人最多可投的赞成票数由该提案类型的额度决定（通常为 1，某些类型允许多个）；成员可以回复 Agent，或使用确认控件。
- 自然语言必须指向明确提案及选项；“我觉得不错”“随便”“赞成第二个但预算再看看”等模糊表达需确认后才记录。
- 投票成功后返回候选、赞成记录时间，以及该投票人剩余的赞成额度。
- 建议默认：截止前允许本人改票，只保留最后一次有效选择用于计票，保留历史供核对。
- 禁止代投、重复计票与截止后补票；未投票与弃权分别展示，不再有反对类别。
- Agent 回答提案问题不得提前泄露仅限特定处理者访问的隐私材料。

**R13 · 资格、权重与利益冲突（权重与回避已从 MVP 延后）**
...
**R14 · 一期建议计票规则（已从 MVP 延后，需组织确认）**

已确认的 MVP 计票规则比本节更简单：只投赞成票、赞成票最高者通过、全员弃权不产生赢家（C08）。下面的加权门槛规则是延后的加权阶段可直接讨论的方案，不代表组织已采用，也不得套用到 MVP 计票上：
...
**后续 Discord 场景（不属于 P0 MVP）：**小林加入 Discord，表示对 AI 安全感兴趣。Agent 推荐真实存在的免费资源、近期活动或课程；没有合适活动时如实说明，并提供兴趣登记与 Contributor 申请入口。该入口不提供 Board 投票、资金或活动审核权限。
...
**场景：**Agent 提前发送三份提案摘要，到点开放投票。成员在额度内回复“赞成 EV-012 和 EV-013，EV-014 弃权”，收到逐项确认与剩余批准额度。
...
| MVP 的 Board 批准投票（§2.3） | 只投赞成票：每名有资格董事权重相同、没有反对选项、弃权不产生赞成票、适用按类型配置的每人批准额度，赞成票最高者通过；全员弃权不产生赢家，最高票并列时也不宣布正式赢家；通过的投票只是决策记录，永不等于付款 |
...
| AC07 | 按已确认的「只投赞成票」规则正确处理：统计赞成票、弃权视为不投赞成票、遵守按类型的每人批准额度、全员弃权报告为不产生赢家，最高票并列时绝不宣布正式赢家，并给出解释。结果公布后按 C15 记录反馈并处理修订：只改标题或摘要的普通修订由 Agent 自行接受并使其生效；涉及预算、地点、日程、人员或活动主流程的修订在记录到现任 Board Member 批准之前必须拒绝。 |
...
AC09–AC14、AC18–AC20 属于 §2.3 中延后的部分，是后续阶段的验收标准；MVP 切片覆盖 AC01–AC04，以及 AC05–AC07、AC16–AC17 中其四个步骤实际涉及的部分。
...
| 决策 | 建议或选项 | 最迟确定阶段 |
| --- | --- | --- |
| 首个聊天平台 | 已确定：Slack（2026-09-24 确认） | 已决定 |
| 评选频率与时区 | 建议双周，明确组织时区 | 真实评选前 |
| 投票窗口及提前通知 | 建议 45 分钟，提前至少 24 小时发简报 | 真实评选前 |
| 参与要求、门槛、回避及预算竞争规则 | MVP 已确定：只投赞成票、每名有资格董事权重相同、弃权不产生赞成票、适用按类型的每人批准额度、赞成票最高者通过、全员弃权不产生赢家；加权投票、法定人数、回避与预算竞争（R13–R15）延后 | MVP 已于 2026-09-24 决定 |
| 结果公布后对已通过提案的修订 | 已确定：只改标题或摘要的普通修订由 Agent 自行接受并生效，无需另行取得 Board 批准；预算、地点、人员与活动主流程（当前实现含日程）须先记录到一名现任 Board Member 的批准。仍待确认：是否允许有效 Contributor 作为反馈作者——数据库行允许，当前注册的工具要求现任董事 | MVP 已于 2026-09-24 决定；作者范围问题在真实的事后修订前确认 |
| 按提案类型的候选名额上限 | 已确认按类型配置，并以有资格投票人数为基准；Events 示例为 10。各类型具体数字仍待登记 | 首次组装真实名单前 |
| 按提案类型的每人批准额度 | 已确认按类型配置：通常为 1，某些类型允许多个。各类型具体额度仍待登记 | 首次组装真实名单前 |
| 平票规则 | 尚未确认：最高票并列不得宣布正式赢家，也不得私自打破；可选“不产生赢家”“主席投决定票”或其他规则。全员弃权已定为不产生赢家 | 真实投票前 |
| Board 名单与投票名单 | 董事来自组织数据库；明确维护人及名单冻结时点 | 真实投票前 |
| 零预算授权活动范围 | 从常规小型教育和讨论活动开始，列明例外 | 自动批准前 |
| Contributor 资格与关联方式 | 沿用既有资格制度；把 Slack 团队／用户对关联到社群记录，并明确谁有权核验 | 正式提案前 |
| 经费来源、币种与快照责任人 | 只读取最新一条人工录入的可用资金快照；明确录入人与币种 | Agent 展示资金前 |
